...
```{r}
reg <- locpolreg(x=Yr, y=lgWeight, h=bandwidth$k.cv) 

mx <- reg$mtgr

z <- log((lgWeight-mx)^2)

q <- locpolreg(Yr, z, h=bandwidth$k.cv)
...
data <- data.frame(Yr, lgWeight, mx, e, banda, -banda, Index)

residualsplot <- ggplot(data, aes(e, Yr)) + geom_point() + geom_line(aes(Index, sigma2)) # It's not working well
residualsplot

ggplot(data, aes(e, Yr)) + geom_point() + geom_line(aes(sigma2, Index)) + ggtitle("Residuals plot with sigma estimated function")


mxplot <- ggplot(data, aes(Index, mx)) + geom_line() + geom_line(data = data, aes(Index, mx + banda), colour = 'red', linetype = "dashed")+ geom_line(data = data, aes(Index, mx - banda), colour = 'red', linetype = "dashed") + ggtitle("Non-parametric estimated m values with confidence bands")
mxplot
