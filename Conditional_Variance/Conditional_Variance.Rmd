...
```
```{r}
fit_sm<- sm.regression(x=lgWeight,y=Yr,h=h.dpill)
```
...
h.dpill_q <- dpill(x=fit_sm$eval.points,y=z,gridsize=101,
                 range.x=range(lgWeight))
q_hat_sm<- sm.regression(x=lgWeight,y=z,h=h.dpill_q)
```
Estimate sigma^2
```{r}
...
g <- ggplot(df, aes(fit_sm$eval.points, fit_sm$estimate)) + geom_line() + geom_line(data = df, aes(fit_sm$eval.points, fit_sm$estimate+ bands), colour = 'red')+ geom_line(data = df, aes(fit_sm$eval.points, fit_sm$estimate- bands), colour = 'red')
g
```
