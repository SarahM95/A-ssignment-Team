...
=======
cl <- as.factor(spam_train[, response])
knn_spam <- knn(spam_train[, explanatory], spam_test[, explanatory], cl = cl, k = 1, prob = TRUE)
...
>>>>>>> fecdc88e313e94e81d369b844937a2d85e0d958c
```
...
As one can see the both the regression using Maximum Likelihood and the one using Lasso are very similar as well as having a high sensitivity ans specificity rate. Knn however is not doing equally good but the trend is similar.
