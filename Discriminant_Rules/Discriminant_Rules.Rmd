...
cv_glmnet_spam <- cv.glmnet(as.matrix(spam_train[, explanatory]), spam_train$spam.01, family = "binomial")
...
knn_spam <- knn(spam_train, spam_test, cl, k=1)
...
library(pROC)
# Complicated version: 
df_glm <- data.frame(response = spam_train$spam.01, predicted = round(glm_spam$fitted.values, 0))
