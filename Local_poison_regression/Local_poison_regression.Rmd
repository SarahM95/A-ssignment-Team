...
```{r}
# Bandwidth choice in the local logistic regression by leave-one-out cross-validation.
...
h.cv.sm.poison <- function(x,y,rg.h=NULL,l.h=10,method=loglik.CV){
   ...
   if (is.null(rg.h)){
      ...
      rg.h <- range(hh)*c(1/1.1, 1.5)# I have change it back, I think is better if we leave the teacher numbers.
...
loglik.CV <- function(x,y,h){
  ...
  #faculty <- sapply(1:n, function(i, y){prod(1:y[i])}, y) # do we need this? no not really, its for the scaling factor below but we are not using it
...
h.cv.sm.poison(countries$Life.expec, le.fm.r)
```

Plot of the cross-validation as a function of Bandwidth.

```{r}
h.CV.loglik <- h.cv.sm.poison(countries$Life.expec,le.fm.r,rg.h=c(1,3),method=loglik.CV)
plot(h.CV.loglik$h,h.CV.loglik$cv.h)
lines(h.CV.loglik$h,h.CV.loglik$cv.h)
title("Bandwidth cross - validation plot")
```

In this plot it can be seen that the best loglikelihood is a number a bit higher than 2.5, which is where the slope turns to 0.

Bandwidth cross-validation log-likelihood plot.
