...
h.cv.sm.poison <- function(x,y,rg.h=NULL,l.h=10,method=loglik.CV){
   ...
   if (is.null(rg.h)){
      ...
      rg.h <- range(hh)*c(1/2, 2)# why this change? Just when testing to get larger span of h, can probably be changed back
...
y<- le.fm.r
h=500 # why 500? no reason, could be 1000, or 5000. But 500 is sufficient to get a lot of values :) 
i=3
n = length(x)
#sm.poisson(x=x[-i],y=y[-i],h=h,eval.points=x[i],display="none")$estimate
  pred <- sapply(1:n, 
