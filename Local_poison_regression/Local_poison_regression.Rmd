...
h.cv.sm.poison <- function(x,y,rg.h=NULL,l.h=10,method=loglik.CV){
   ...
   if (is.null(rg.h)){
      ...
      rg.h <- range(hh)*c(1/2, 2)# why this change?
...
loglik.CV <- function(x,y,h){
  ...
  return(-sum(y*log(pred)-pred)) #-log(faculty) only a scale # why this change?
  # return(-sum( y*log(pred/(1-pred)) + log(1-pred) )/n)# I understand why you change log(pred/(1-pred)) for log(pred) but I don't really understand the change of the second part.
}
