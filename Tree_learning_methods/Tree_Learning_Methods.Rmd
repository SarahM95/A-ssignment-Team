...
In the summary we can see that the only variable that has missing values is the $x71$ with a percentage of $13.97$%. We consider that is not a reason for deleting the variable so we decided treat the it substituing the NA values for the mean. 
...
For checking the variability of the data and how it is distributed we did a violin plots. As we have a lot of variables we split the data by rank in order to observe it better.
...
We have some variables that are linearly correlated between them. 

Next we do is a Cluster Dendrogram in order to check graphically the distances between the variables.
...
Looking to the principal components it's seen that with just three components we get almost the 95% of the data variation.

Next step is check the importance that each variable has in each principal component, we are just going to check it for the first threee components. 
...
In this plots it can be seen that there is a lot of variables that don't contribut much on this principal components. For knowing which variables are the ones that are not that important and that can be consideret for removing we sum the contribution of all three variables and then we look which ones has less than $0.05$.
...
Checking this we can see that variables **`r nsign`** aren't important for explaining much of the variance of this dataset.

As it's seen there is a lot of variables thats seems to be substracted from the original data but the decision that we make is to keep them and use this analysis to check if when we do the prunned tree and the random forest analysisi the variables that are not used are the same.
...
plot(CART_single_tree_2_2$finalModel)
text(CART_single_tree_2_2$finalModel)
```


#3 Pruned single tree, oldfasion way :) :
