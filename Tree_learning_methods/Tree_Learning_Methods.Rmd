...
In this dendrogram it can be seen that there are a lot of clusters meaning a lot of the variables are close to each other with respect to the correlation mean distance. So it's saying, as we commented before that there is a lot of variables that are close correlated.
...
In this plots it can be seen that there is a lot of variables that don't contribut much to the principal components. To know which variables are less important we sum the contribution of the variables to the three first principal components and looked at  which ones had a sum less than $0.05$. These are the ones that can be considered removed if one would want to simplify the analytics. 
...
#Its giving an error when i try to run it but I don't know if its something of my computer...
...
###Both models are using the same variables?? Maybe we should include that in the prunned tree and compare both with that too? I can do that tomorrow if you want.
...
Concerning the use of the function :
  ...
  - we use stumps to study iteration influence. In this case, we then choose interaction.depth equal to 1.
...
As we can see, the accuracy rate is really higher on training data (around 85%) set than on testing data set (around 75%), which is expected due to the fact that the model is fitted on training data set.

We should retain the missclassification rate of testing data set. We can notice that the model for 2000 iterations with stumps as trees is more efficient than pruned single tree model but less efficient that with a forest trees model.
...
On this plot, as expected again, we can see that missclassification in function of trees depth is more or less a decreasing function.

In the best condition (which means with trees depth equal to 16) we can reach 81% of accuracy rate. Then, we can put foward that this algorithm is the most efficient (for memory, random forest model reacht 79% of accuracy rate).
  
To conclude we can say that trees depth & iteration number are two factor which can be used to optimize our classification. Nevertheless, it important to be careful because it is two factors very greedy in ressource.